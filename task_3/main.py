...
def parse_log_line(line: str) -> dict:
    pattern = r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) (\w+) (.+)$" #pattern of logfile line

    match = re.match(pattern, line) #group line parts
    if match:
        return {
        ...
        raise ValueError("Неправильний формат рядку у файлі")
...
def main():
    ...
    if len(sys.argv) >= 3:   #for filter function 
...
#py task_3/main.py task_3/logfile.txt
#py task_3/main.py task_3/logfile.txt error